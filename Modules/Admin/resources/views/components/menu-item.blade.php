...
<li data-id="{{ $menu->id }}" class="relative">
    <div class="bg-white border border-gray-200 rounded-lg shadow-sm p-3 flex items-center justify-between group hover:border-indigo-300 transition-colors">
        ...
        <div class="flex items-center gap-3 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <button wire:click="toggleStatus({{ $menu->id }})" class="{{ $menu->is_active ? 'text-green-500' : 'text-gray-300' }} hover:scale-110 transition">
                <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/></svg>
            </button>

            <a href="{{ route('admin.menus.edit', $menu->id) }}" class="text-indigo-600 hover:text-indigo-800 p-1">
                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg>
            </a>

            <button wire:confirm="Xóa menu này?" wire:click="delete({{ $menu->id }})" class="text-red-400 hover:text-red-600 p-1">
