<div>
    <div class="flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
        ...
        <a href="{{ route('admin.product-categories.create') }}" class="inline-flex items-center gap-x-1.5 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-500 transition">
    ...
    <div class="bg-white rounded-xl shadow-sm ring-1 ring-gray-900/5 overflow-hidden relative">
        <div wire:loading.flex wire:target="setType, delete, toggleStatus" class="absolute inset-0 bg-white/60 z-10 items-center justify-center backdrop-blur-[1px]">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50/50">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wide text-gray-500">Tên danh mục</th>
                        <th class="px-6 py-3 text-center text-xs font-medium uppercase tracking-wide text-gray-500">Thứ tự</th>
                        <th class="px-6 py-3 text-center text-xs font-medium uppercase tracking-wide text-gray-500">Trạng thái</th>
                        <th class="px-6 py-3 text-right text-xs font-medium uppercase tracking-wide text-gray-500">Hành động</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-200 bg-white">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    <div class="h-10 w-10 flex-shrink-0 mr-4">
                                        @if($parent->image)
                                            <img class="h-10 w-10 rounded-lg object-cover border border-gray-200" src="{{ asset('storage/'.$parent->image) }}">
                                        @else
                                    ...
                                    <div>
                                        <div class="font-bold text-gray-900 group-hover:text-indigo-600 transition">{{ $parent->name }}</div>
                                        <div class="text-xs text-gray-500 font-mono">/{{ $parent->slug }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-center text-sm font-bold text-gray-500">{{ $parent->sort_order }}</td>
                            <td class="px-6 py-4 text-center">
                                <button wire:click="toggleStatus({{ $parent->id }})" class="relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none {{ $parent->is_active ? 'bg-green-500' : 'bg-gray-200' }}">
                                    <span class="pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out {{ $parent->is_active ? 'translate-x-4' : 'translate-x-0' }}"></span>
                                </button>
                            </td>
                            <td class="px-6 py-4 text-right text-sm font-medium">
                                <a href="{{ route('admin.product-categories.edit', $parent->id) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">Sửa</a>
                                <button wire:confirm="Xóa danh mục này sẽ ảnh hưởng đến sản phẩm?" wire:click="delete({{ $parent->id }})" class="text-red-600 hover:text-red-900">Xóa</button>
                            </td>
                        </tr>
                        ...
                        @foreach($parent->children as $child)
                            <tr class="bg-slate-50 hover:bg-slate-100 transition">
                                <td class="px-6 py-3 pl-16">
                                    <div class="flex items-center relative">
                                        ...
                                        <div class="h-8 w-8 flex-shrink-0 mr-3">
                                            @if($child->image) <img class="h-8 w-8 rounded-md object-cover" src="{{ asset('storage/'.$child->image) }}"> @endif
                                        </div>
                                ...
                                <td class="px-6 py-3 text-right text-sm font-medium">
                                    <a href="{{ route('admin.product-categories.edit', $child->id) }}" class="text-indigo-600 hover:text-indigo-900 mr-2">Sửa</a>
                                    <button wire:confirm="Xóa?" wire:click="delete({{ $child->id }})" class="text-red-600 hover:text-red-900">Xóa</button>
                                </td>
                            </tr>
                            ...
                            @foreach($child->children as $grandChild)
                                <tr class="bg-white hover:bg-gray-50 transition">
                                    ...
                                    <td class="px-6 py-2 text-center text-xs text-gray-400">{{ $grandChild->sort_order }}</td>
                                    <td class="px-6 py-2 text-center"><div class="h-2 w-2 rounded-full {{ $grandChild->is_active ? 'bg-green-400' : 'bg-gray-300' }} mx-auto"></div></td>
                                    <td class="px-6 py-2 text-right text-xs font-medium">
                                        <a href="{{ route('admin.product-categories.edit', $grandChild->id) }}" class="text-indigo-600 hover:text-indigo-900 mr-2">Sửa</a>
                                        <button wire:confirm="Xóa?" wire:click="delete({{ $grandChild->id }})" class="text-red-600 hover:text-red-900">Xóa</button>
                                    </td>
                                </tr>
                            @endforeach
                        @endforeach
                    ...
                    @empty
                        <tr><td colspan="4" class="px-6 py-10 text-center text-gray-500 italic">Chưa có danh mục nào cho loại này.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
