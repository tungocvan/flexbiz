<div class="container mx-auto px-4 mb-24">
    <div class="relative bg-gray-900 rounded-[2.5rem] p-8 md:p-16 overflow-hidden shadow-2xl">
        ...
        {{-- Background Decor (Các vòng tròn mờ ảo tạo chiều sâu) --}}
        ...
        <div class="relative z-10 flex flex-col md:flex-row items-center justify-between gap-12">
            ...
            <div class="w-full md:w-1/2 text-center md:text-left">
                ...
                <h2 class="text-3xl md:text-4xl lg:text-5xl font-black text-white leading-tight mb-4">
                    Mở khóa ưu đãi <span class="text-blue-400">10%</span> cho đơn hàng đầu tiên.
                </h2>
                <p class="text-gray-400 text-lg md:max-w-md">
                    Đăng ký để nhận tin tức về bộ sưu tập mới, mẹo phối đồ và các ưu đãi độc quyền chỉ dành cho thành viên.
                </p>
            </div>
            ...
            <div class="w-full md:w-1/2 max-w-lg">
                @if($subscribed)
                    ...
                    <div class="bg-white/10 backdrop-blur-md border border-green-500/30 p-8 rounded-2xl text-center animate-fade-in-up">
                        ...
                        <p class="text-gray-300">Cảm ơn bạn đã tham gia. Mã giảm giá đã được gửi vào email của bạn.</p>
                    </div>
                @else
                    {{-- Form State --}}
                    <form wire:submit.prevent="subscribe" class="relative">
                        <div class="flex flex-col gap-4">
                            ...
                            {{-- Error Message --}}
                            @error('email')
                                <span class="text-red-400 text-sm font-medium pl-2 flex items-center gap-1">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                    {{ $message }}
                                </span>
                            @enderror

                            <button type="submit" class="w-full py-4 bg-white text-gray-900 font-bold rounded-2xl hover:bg-blue-50 transition-all duration-300 shadow-[0_0_20px_rgba(255,255,255,0.2)] hover:shadow-[0_0_30px_rgba(255,255,255,0.4)] transform hover:-translate-y-1 relative overflow-hidden disabled:opacity-70 disabled:cursor-not-allowed">
                                {{-- Loading Spinner --}}
                                <span wire:loading wire:target="subscribe" class="absolute inset-0 flex items-center justify-center bg-white">
...
{{-- CSS Animation Keyframes cho blob background (Có thể thêm vào app.css) --}}
<style>
    @keyframes blob {
